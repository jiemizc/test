...
public class NettyWorkerServer<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
	 ...
	 *            Mapper context
	 */
	public NettyWorkerServer(
			ImmutableClassesGiraphConfiguration<I, V, E, M> conf,
			CentralizedServiceWorker<I, V, E, M> service,
			Mapper<?, ?, ?, ?>.Context context, WorkerContext workerContext) {
		this.conf = conf;
		this.service = service;

		serverData = new ServerData<I, V, E, M>(service, conf,
				createMessageStoreFactory(), context, workerContext);
	...
	/**
	 * Decide which message store should be used for current application, and
	 * create the factory for that store
	 *
	...
	private MessageStoreFactory<I, M, MessageStoreByPartition<I, M>> createMessageStoreFactory() {
		...
		if (!useOutOfCoreMessaging) {
			if (conf.useCombiner()) {
			...
			} else if (conf.useOnlineCompute())
				return OnlineComputeMessageStore.newFactory(service, conf);
			else {
				if (LOG.isInfoEnabled()) {
					LOG.info("createMessageStoreFactory: "
				...
				return ByteArrayMessagesPerVertexStore
						.newFactory(service, conf);
			}
		} else {
			if (conf.useOnlineCompute()) {
				long maxMem = Runtime.getRuntime().maxMemory();// bytes
				int maxMessageInMem = MAX_MESSAGES_IN_MEMORY.get(conf);
				int maxMessageSizeInMemory = (int)(maxMem * conf
							.getMsgBufferPercent()/1000);//KB
					
				if (LOG.isInfoEnabled()) {
					LOG.info("createMessageStoreFactory: Using DiskBackedOnlineComputeMessageStore, "
							+ "maxMessagesInMemory = " + maxMessageSizeInMemory + "KB");
				}
				...
				return DiskBackedOnlineComputeMessageStoreByPartition
						.newFactory(service, maxMessageSizeInMemory,
	...
	private void resolveMutations(GraphState<I, V, E, M> graphState) {
		if (conf.isStaticGraph())
			return;
		Multimap<Integer, I> resolveVertexIndices = HashMultimap.create(service
				.getPartitionStore().getNumPartitions(), 100);
		// Add any mutated vertex indices to be resolved
		for (Entry<I, VertexMutations<I, V, E, M>> e : serverData
				.getVertexMutations().entrySet()) {
			I vertexId = e.getKey();
		...
		// Keep track of the vertices which are not here but have received
		// messages
		for (Integer partitionId : service.getPartitionStore()
				.getPartitionIds()) {
			Iterable<I> destinations = serverData.getCurrentMessageStore()
					.getPartitionDestinationVertices(partitionId);
			if (!Iterables.isEmpty(destinations)) {
				Partition<I, V, E, M> partition = service.getPartitionStore()
		...
		VertexResolver<I, V, E, M> vertexResolver = conf
				.createVertexResolver(graphState);
		for (Entry<Integer, Collection<I>> e : resolveVertexIndices.asMap()
				.entrySet()) {
			Partition<I, V, E, M> partition = service.getPartitionStore()
					.getPartition(e.getKey());
			for (I vertexIndex : e.getValue()) {
				Vertex<I, V, E, M> originalVertex = partition
						.getVertex(vertexIndex);

				VertexMutations<I, V, E, M> mutations = null;
				VertexMutations<I, V, E, M> vertexMutations = serverData
						.getVertexMutations().get(vertexIndex);
				...
				Vertex<I, V, E, M> vertex = vertexResolver.resolve(vertexIndex,
						originalVertex, mutations,
