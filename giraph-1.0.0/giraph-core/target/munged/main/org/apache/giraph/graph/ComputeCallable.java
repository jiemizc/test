...
import org.apache.giraph.comm.messages.DiskBackedOnlineComputeMessageStoreByPartition;
import org.apache.giraph.comm.messages.IMWritable;
import org.apache.giraph.comm.messages.MessageStoreByPartition;
import org.apache.giraph.comm.messages.OnlineComputeMessageStore;
import org.apache.giraph.comm.messages.OnlineComputeMessageStore.IdMsgPair;
...
public class ComputeCallable<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
	...
	public Collection<PartitionStats> call() {
		...
		while (!partitionIdQueue.isEmpty()) {
			...
			// LOG.info("start computing partition:"+partitionId);
			boolean isHot = true;
			if (serviceWorker.getPartitionStore() instanceof DiskBackedOnlineComputePartitionStore) {
				DiskBackedOnlineComputePartitionStore<I, V, E, M> pStore = (DiskBackedOnlineComputePartitionStore<I, V, E, M>) serviceWorker
						.getPartitionStore();
				partition = pStore.getPartition(partitionId, 1);
				isHot = pStore.isHot(partitionId);
			} else {
				partition = serviceWorker.getPartitionStore().getPartition(
						partitionId);
			}
			try {
				...
				if (configuration.useOnlineCompute())
					...
					partitionStats = computePartition(partition);
				...
				LOG.info("end compute partition" + partitionId
						+ " memstatus= " + MemoryUtils.getRuntimeMemoryStats());
				// LOG.info("call: Completed " + partitionStatsList.size()
				...
				if (serviceWorker.getPartitionStore() instanceof DiskBackedOnlineComputePartitionStore) {
					DiskBackedOnlineComputePartitionStore<I, V, E, M> pStore = (DiskBackedOnlineComputePartitionStore<I, V, E, M>) serviceWorker
							.getPartitionStore();
					pStore.putPartition(partition, 1);
				} else {
	...
	// @Override
	public Collection<PartitionStats> callRaw() {
		// Thread initialization (for locality)
		this.workerClientRequestProcessor = new NettyWorkerClientRequestProcessor<I, V, E, M>(
				context, configuration, serviceWorker);
		WorkerThreadAggregatorUsage aggregatorUsage = serviceWorker
	...
	/**
	 * compute for memory online compute
	 * 
	 * @param partition
	 * @return
	 * @throws IOException
	 ...
	 */
	private PartitionStats computePartitionForMOC(
			Partition<I, V, E, M> partition) throws IOException,
			...
			for (Vertex<I, V, E, M> vertex : partition) {
				...
				if (!vertex.isHalted()) {
					...
					TimerContext computeOneTimerContext = computeOneTimer
							.time();
					try {
						vertex.sendMessagesForInmemVertex(graphState
								.getSuperstep());
					} finally {
						computeOneTimerContext.stop();
					}
	...
	/**
	 * onlineCompute a single partition for disk-backed partition
	 * 
	 * @param partition
	 *            Partition to compute for disk online compute
	 * @return Partition stats for this computed partition,now is in memory
	 */
	private PartitionStats computePartitionForDOC(
			Partition<I, V, E, M> partition) throws IOException,
			InterruptedException {
		PartitionStats partitionStats = new PartitionStats(partition.getId(),
				0, 0, 0, 0);
		...
		synchronized (partition) {
			...
			// online compute
			if (messageStore instanceof DiskBackedOnlineComputeMessageStoreByPartition) {
				DiskBackedOnlineComputeMessageStoreByPartition msgStoreByPartition = (DiskBackedOnlineComputeMessageStoreByPartition) messageStore;
				Iterable<IMWritable<I, M>> msgs = msgStoreByPartition
						.getPartitionMessages(partition.getId());
				Iterator<IMWritable<I, M>> msgsIter = msgs.iterator();
				// LOG.info("onlinecompute msg for "+partition.getId()+" started");
				while (msgsIter.hasNext()) {
					IMWritable<I, M> IMPair = msgsIter.next();
					I id = IMPair.vertexId;
					M msg = IMPair.message;
					Vertex<I, V, E, M> v = partition.getVertex(id);
					v.onlineComputeForOOCVertex(msg,
							graphState.getSuperstep() - 1);
				}
				// LOG.info("onlinecompute msg for "+partition.getId()+
				// " ended");
			}
			// LOG.info("send msg for "+partition.getId()+ " started");
			// send msgs
			for (Vertex<I, V, E, M> vertex : partition) {
				// Make sure every vertex has this thread's
				// graphState before computing
				...
				if (!vertex.isHalted()) {
					...
					TimerContext computeOneTimerContext = computeOneTimer
							.time();
					try {
						vertex.sendMessagesForOOCVertex(graphState
								.getSuperstep());
					} finally {
						computeOneTimerContext.stop();
					}
		...
		// LOG.info("send msg for "+partition.getId()+ " ended");
		return partitionStats;
	}
}
