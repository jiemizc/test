...
public class MasterAggregatorHandler implements MasterAggregatorUsage, Writable {
	...
	private static final Logger LOG = Logger
			.getLogger(MasterAggregatorHandler.class);
	/**
	 * Map of aggregators. This map is used to store final aggregated values
	 * received from worker owners, and also to read and write values provided
	 * during master.compute.
	 */
	private final Map<String, AggregatorWrapper<Writable>> aggregatorMap = Maps
			.newHashMap();
	/** Aggregator writer */
	 ...
	 *            Giraph configuration
	 * @param progressable
	 *            Progressable used for reporting progress
	 */
	...
	public <A extends Writable> A getAggregatedValue(String name) {
		AggregatorWrapper<? extends Writable> aggregator = aggregatorMap
				.get(name);
		if (aggregator == null) {
	...
	public <A extends Writable> void setAggregatedValue(String name, A value) {
		AggregatorWrapper<? extends Writable> aggregator = aggregatorMap
				.get(name);
		if (aggregator == null) {
			throw new IllegalStateException("setAggregatedValue: "
					+ AggregatorUtils.getUnregisteredAggregatorMessage(name,
							aggregatorMap.size() != 0, conf));
		}
	 ...
	 *
	 * @param name
	 *            Name of the aggregator to check.
	 */
	 ...
	 * @param persistent
	 *            Whether aggregator is persistent or not
	 * @param <A>
	...
	private <A extends Writable> AggregatorWrapper<A> registerAggregator(
		...
		AggregatorWrapper<A> aggregatorWrapper = (AggregatorWrapper<A>) aggregatorMap
				.get(name);
		if (aggregatorWrapper == null) {
			aggregatorWrapper = new AggregatorWrapper<A>(aggregatorClass,
					persistent);
			aggregatorMap.put(name,
	...
	public void prepareSuperstep(MasterClient masterClient) {
		...
		for (AggregatorWrapper<Writable> aggregator : aggregatorMap.values()) {
			if (aggregator.isPersistent()) {
				aggregator.aggregateCurrent(aggregator
						.getPreviousAggregatedValue());
			}
			aggregator.setPreviousAggregatedValue(aggregator
					.getCurrentAggregatedValue());
	...
	public void finishSuperstep(MasterClient masterClient) {
		...
		try {
			for (Map.Entry<String, AggregatorWrapper<Writable>> entry : aggregatorMap
					.entrySet()) {
				masterClient.sendAggregator(entry.getKey(), entry.getValue()
						.getAggregatorClass(), entry.getValue()
						.getPreviousAggregatedValue());
				progressable.progress();
			}
	 ...
	 * only once, by its owner. We don't need to count the number of these
	 * requests because global superstep barrier will happen after workers
	 * ensure all requests of this type have been received and processed by
	 * master.
	 *
	 * @param aggregatedValuesInput
	 *            Input in which aggregated values are written in the following
	 *            format: number_of_aggregators name_1 value_1 name_2 value_2
	 *            ...
	 * @throws IOException
	 */
	public void acceptAggregatedValues(DataInput aggregatedValuesInput)
			throws IOException {
		int numAggregators = aggregatedValuesInput.readInt();
		for (int i = 0; i < numAggregators; i++) {
			String aggregatorName = aggregatedValuesInput.readUTF();
			AggregatorWrapper<Writable> aggregator = aggregatorMap
					.get(aggregatorName);
			if (aggregator == null) {
				throw new IllegalStateException("acceptAggregatedValues: "
	 ...
	 *
	 * @param superstep
	 *            Superstep which just finished
	 * @param superstepState
	 *            State of the superstep which just finished
	 */
    ...
    try {
      ...
      long superstepCode = (superstepState == SuperstepState.ALL_SUPERSTEPS_DONE) ?
              AggregatorWriter.LAST_SUPERSTEP : superstep;
      aggregatorWriter.writeAggregator(iter,superstepCode);
    } catch (IOException e) {
