...
import org.apache.giraph.comm.requests.SendWorkerValuesRequest;
...
/**
 * Aggregate requests and sends them to the thread-safe NettyClient. This class
 * is not thread-safe and expected to be used and then thrown away after a phase
 * of communication has completed.
 *
 * @param <I>
 *            Vertex id
 * @param <V>
...
public class NettyWorkerClientRequestProcessor<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
		implements WorkerClientRequestProcessor<I, V, E, M> {
	/** Class logger */
	private static final Logger LOG = Logger
			.getLogger(NettyWorkerClientRequestProcessor.class);
	...
	private final SendMutationsCache<I, V, E, M> sendMutationsCache = new SendMutationsCache<I, V, E, M>();
	...
	public NettyWorkerClientRequestProcessor(
		...
		// Per-Superstep Metrics.
		// Since this object is not long lived we just initialize the metrics
		// here.
		...
		final Gauge<Long> totalRequests = smr.getGauge(
				MetricNames.TOTAL_REQUESTS, new Gauge<Long>() {
					@Override
					public Long value() {
	...
	public boolean sendMessageRequest(I destVertexId, M message) {
		PartitionOwner owner = serviceWorker
				.getVertexPartitionOwner(destVertexId);
		...
		int workerMessageSize = sendMessageCache.addMessage(workerInfo,
				partitionId, destVertexId, message);

		// Send a request if the cache of outgoing message to
		// the remote worker 'workerInfo' is full enough to be flushed
		if (workerMessageSize >= maxMessagesSizePerWorker) {
			PairList<Integer, ByteArrayVertexIdMessages<I, M>> workerMessages = sendMessageCache
	...
	/**
	 * All the broadcastValueRequest called by a task have the same
	 * partitionOwner of the task itself
	 * 
	 * @param src_id
	 *            vertexId
	 * @param msg
	 ...
	 */
	@Override
	public boolean broadcastValueRequest(I src_id, M msg) {
		PartitionOwner owner = serviceWorker.getVertexPartitionOwner(src_id);
		int partitionId = owner.getPartitionId();
		WorkerInfo localWorkerInfo = owner.getWorkerInfo();

		List<WorkerInfo> workerInfos = serviceWorker.getWorkerInfoList();
		totalMsgsSentInSuperstep += workerInfos.size();

		// add message to the partition of src_id
		int workerMessageSize = sendMessageCache.addMessage(localWorkerInfo,
				partitionId, src_id, msg);

		if (workerMessageSize >= maxMessagesSizePerWorker) {
			PairList<Integer, ByteArrayVertexIdMessages<I, M>> workerMessages = sendMessageCache
					.removeWorkerMessages(localWorkerInfo);
			WritableRequest writableRequest = new SendWorkerValuesRequest<I, M>(
					workerMessages);
			// TODO: use netty multicast plz.
			for (WorkerInfo wi : workerInfos) {
				doRequest(wi, writableRequest);
			}
			return true;
		}
		return false;
	...
	public void sendPartitionRequest(WorkerInfo workerInfo,
		...
		WritableRequest vertexRequest = new SendVertexRequest<I, V, E, M>(
				partition);
		vertexRequest.setConf(configuration);
	 ...
	 *
	 * @param workerInfo
	 *            Worker to send the partition messages to
	 * @param partition
	 *            Partition whose messages to send
	 */
	private void sendPartitionMessages(WorkerInfo workerInfo,
			Partition<I, V, E, M> partition) {
		final int partitionId = partition.getId();
		MessageStoreByPartition<I, M> messageStore = serverData
				.getCurrentMessageStore();
		ByteArrayVertexIdMessages<I, M> vertexIdMessages = new ByteArrayVertexIdMessages<I, M>();
		vertexIdMessages.setConf(configuration);
		vertexIdMessages.initialize();
		for (I vertexId : messageStore
			...
			if (vertexIdMessages.getSize() > maxMessagesSizePerWorker) {
				WritableRequest messagesRequest = new SendPartitionCurrentMessagesRequest<I, V, E, M>(
						partitionId, vertexIdMessages);
				doRequest(workerInfo, messagesRequest);
				vertexIdMessages = new ByteArrayVertexIdMessages<I, M>();
				vertexIdMessages.setConf(configuration);
				vertexIdMessages.initialize();
			}
		}
	...
	public void sendVertexRequest(PartitionOwner partitionOwner,
		...
		Partition<I, V, E, M> partition = sendPartitionCache.addVertex(
	...
	@Override
	public void addEdgeRequest(I vertexIndex, Edge<I, E> edge)
			throws IOException {
		PartitionOwner partitionOwner = serviceWorker
				.getVertexPartitionOwner(vertexIndex);
		...
		// Add the message to the cache
		int partitionMutationCount = sendMutationsCache.addEdgeMutation(
				partitionId, vertexIndex, edge);

		sendMutationsRequestIfFull(partitionId, partitionOwner,
				partitionMutationCount);
	}
	...
	public boolean sendEdgeRequest(I sourceVertexId, Edge<I, E> edge)
			throws IOException {
		PartitionOwner owner = serviceWorker
				.getVertexPartitionOwner(sourceVertexId);
		WorkerInfo workerInfo = owner.getWorkerInfo();
		final int partitionId = owner.getPartitionId();
		if (LOG.isTraceEnabled()) {
		...
		// Add the message to the cache
		int workerEdgesSize = sendEdgeCache.addEdge(workerInfo, partitionId,
				sourceVertexId, edge);

		// Send a request if the cache of outgoing edges to the remote worker is
		// full
		if (workerEdgesSize >= maxEdgesSizePerWorker) {
			PairList<Integer, ByteArrayVertexIdEdges<I, E>> workerEdges = sendEdgeCache
					.removeWorkerEdges(workerInfo);
	 ...
	 * @param partitionId
	 *            Partition id
	 * @param partitionOwner
	 *            Owner of the partition
	 * @param partitionMutationCount
	 *            Number of mutations for this partition
	 */
	private void sendMutationsRequestIfFull(int partitionId,
			PartitionOwner partitionOwner, int partitionMutationCount) {
		// Send a request if enough mutations are there for a partition
		if (partitionMutationCount >= maxMutationsPerPartition) {
			Map<I, VertexMutations<I, V, E, M>> partitionMutations = sendMutationsCache
					.removePartitionMutations(partitionId);
			WritableRequest writableRequest = new SendPartitionMutationsRequest<I, V, E, M>(
	...
	@Override
	public void removeEdgesRequest(I vertexIndex, I destinationVertexIndex)
			throws IOException {
		PartitionOwner partitionOwner = serviceWorker
				.getVertexPartitionOwner(vertexIndex);
		int partitionId = partitionOwner.getPartitionId();
		if (LOG.isTraceEnabled()) {
			LOG.trace("removeEdgesRequest: Removing edge "
					+ destinationVertexIndex + " for index " + vertexIndex
	...
	public void addVertexRequest(Vertex<I, V, E, M> vertex) throws IOException {
		...
		// Add the message to the cache
		int partitionMutationCount = sendMutationsCache.addVertexMutation(
				partitionId, vertex);
	...
	public void removeVertexRequest(I vertexIndex) throws IOException {
		...
		// Add the message to the cache
		int partitionMutationCount = sendMutationsCache.removeVertexMutation(
				partitionId, vertexIndex);
	...
	public void flush() throws IOException {
		...
		for (Map.Entry<PartitionOwner, Partition<I, V, E, M>> entry : sendPartitionCache
				.getOwnerPartitionMap().entrySet()) {
			sendPartitionRequest(entry.getKey().getWorkerInfo(),
					entry.getValue());
		}
		sendPartitionCache.clear();

		// Execute the remaining sends messages (if any)
		PairList<WorkerInfo, PairList<Integer, ByteArrayVertexIdMessages<I, M>>> remainingMessageCache = sendMessageCache
				.removeAllMessages();
		PairList<WorkerInfo, PairList<Integer, ByteArrayVertexIdMessages<I, M>>>.Iterator iterator = remainingMessageCache
				.getIterator();
		List<WorkerInfo> workerInfos = serviceWorker.getWorkerInfoList();
		...
		PairList<WorkerInfo, PairList<Integer, ByteArrayVertexIdEdges<I, E>>> remainingEdgeCache = sendEdgeCache
				.removeAllEdges();
		PairList<WorkerInfo, PairList<Integer, ByteArrayVertexIdEdges<I, E>>>.Iterator edgeIterator = remainingEdgeCache
				.getIterator();
		...
		Map<Integer, Map<I, VertexMutations<I, V, E, M>>> remainingMutationsCache = sendMutationsCache
				.removeAllPartitionMutations();
		for (Map.Entry<Integer, Map<I, VertexMutations<I, V, E, M>>> entry : remainingMutationsCache
				.entrySet()) {
			WritableRequest writableRequest = new SendPartitionMutationsRequest<I, V, E, M>(
					entry.getKey(), entry.getValue());
			PartitionOwner partitionOwner = serviceWorker
					.getVertexPartitionOwner(entry.getValue().keySet()
							.iterator().next());
			doRequest(partitionOwner.getWorkerInfo(), writableRequest);
		}
	}
	 ...
	 * @param workerInfo
	 *            Worker info
	 * @param writableRequest
	 *            Request to either submit or run locally
	 */
	private void doRequest(WorkerInfo workerInfo,
		...
		if (serviceWorker.getWorkerInfo().getTaskId() == workerInfo.getTaskId()) {
			...
			workerClient.sendWritableRequest(workerInfo.getTaskId(),
					writableRequest);
			remoteRequests.inc();
		}
	}
