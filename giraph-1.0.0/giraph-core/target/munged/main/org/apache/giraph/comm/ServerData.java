...
public class ServerData<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable> {
	...
	/**
	 * Message store for incoming messages (messages which will be consumed in
	 * the next super step)
	 */
	 ...
	 */
	private final ConcurrentHashMap<I, VertexMutations<I, V, E, M>> vertexMutations = new ConcurrentHashMap<I, VertexMutations<I, V, E, M>>();
	/**
	...
	public ServerData(
		...
		this(service, conf, messageStoreFactory, context);
	...
	public ServerData(
		...
		if (GiraphConstants.USE_OUT_OF_CORE_GRAPH.get(conf)) {
			if (GiraphConstants.USE_ONLINE_COMPUTE.get(conf))
				partitionStore = new DiskBackedOnlineComputePartitionStore<I, V, E, M>(
						conf, context);
			else
				partitionStore = new DiskBackedPartitionStore<I, V, E, M>(conf,
						context);
		} else {
			partitionStore = new SimplePartitionStore<I, V, E, M>(conf, context);
		}
	...
	public void receiveVertexValue(
			ByteArrayVertexIdMessages<I, M> byteArrayVertexIdMsgs) {
	}
