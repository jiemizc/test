...
import org.apache.giraph.comm.messages.DiskBackedIncomeMessageStoreByPartition;
import org.apache.giraph.comm.messages.InMemMessagePair;
import org.apache.giraph.comm.messages.IncomeFileMessageStore;
...
public class GraphTaskManager<I extends WritableComparable, V extends Writable,
  ...
  */
  public void execute() throws IOException, InterruptedException {
    ...
    //hot analysis if using onlinePartitionStore
    PartitionStore<I,V,E,M> pStore = serviceWorker.getPartitionStore();
    if(pStore instanceof DiskBackedOnlineComputePartitionStore){
    	DiskBackedOnlineComputePartitionStore dStore = 
    			(DiskBackedOnlineComputePartitionStore) pStore;
    	if(conf.useHotPartition())
    		dStore.hotRepartition();
    	else
    		dStore.hotAnalysis();
    }
    ...
    do {
      ...
      //TODO:
//      if(conf.useIncomingEdgeMode() && conf.usePartialCompute()){
//    	  processMessagePartition(context, partitionStatsList, graphState,
//    	          messageStore, numPartitions);
//      }else{
	      if (numPartitions > 0) {
...
//      }
 ...
 /**
  ...
  private void processGraphPartitions(final Mapper<?, ?, ?, ?>.Context context,
    ...
    if(serviceWorker.getPartitionStore() instanceof 
    		DiskBackedOnlineComputePartitionStore){
    	DiskBackedOnlineComputePartitionStore store = (DiskBackedOnlineComputePartitionStore)
    			serviceWorker.getPartitionStore();
    	int coldid = store.getInMemColdParititionId();
    	//if hot partitioned
    	if(coldid!=-1)
    		computePartitionIdQueue.add(coldid);
    	for (Integer partitionId :
    	      serviceWorker.getPartitionStore().getPartitionIds()) {
    	      if(partitionId!=coldid)
