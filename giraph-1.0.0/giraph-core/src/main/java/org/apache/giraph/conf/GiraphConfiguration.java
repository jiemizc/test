 ...
 * ImmutableClassesGiraphConfiguration that provides the configuration plus the
 * immutable classes.
 */
public class GiraphConfiguration extends Configuration implements
		GiraphConstants {
	/**
	 ...
	 * 
	 * @param vertexValueFactoryClass
	 *            Creates default vertex values
	 */
	 ...
	 * 
	 * @param outEdgesClass
	 *            Determines the way edges are stored
	 */
	...
	/**
	 * True if the {@link org.apache.giraph.edge.OutEdges} implementation copies
	 * the passed edges to its own data structure, i.e. it doesn't keep
	 * references to Edge objects, target vertex ids or edge values passed to
	 * add() or initialize(). This makes it possible to reuse edge objects
	 * passed to the data structure, to minimize object instantiation (see for
	 * example EdgeStore#addPartitionEdges()).
	 * 
	...
	public boolean isInSynchronousMode() {
		return IS_IN_SYNC_MODE.get(this);
	}
	public void setSynchronousMode(boolean flag){
		IS_IN_SYNC_MODE.set(this, flag);
	}
	public boolean useHotPartition(){
		return HOT_PARTITION.get(this);
	}
	public void setUseHotPartition(boolean flag){
		HOT_PARTITION.set(this, flag);
	}
	public boolean useEdgeSeparation(){
		return EDGE_SEPARATION.get(this);
	}
	public void setUseEdgeSeparation(boolean flag){
		EDGE_SEPARATION.set(this, flag);
	}
	public boolean useOnlineCompute(){
		return USE_ONLINE_COMPUTE.get(this);
	}
	public void setUseOnlineCompute(boolean flag){
		USE_ONLINE_COMPUTE.set(this, flag);
	}
	public float getMsgBufferPercent(){
		return MESSAGE_BUFFER_PERCENT.get(this);
	}
	 ...
	 * objects passed to it. This makes it possible to reuse vertex objects
	 * passed to the data structure, to minimize object instantiation.
	 * 
	 * @return True iff we can reuse the vertex objects
	 */
	public boolean reuseVertexObjects() {
		return ReusesObjectsPartition.class
				.isAssignableFrom(getPartitionClass());
	}
	 ...
	 * 
	 * @param masterObserverClass
	 *            MasterObserver class to add.
	 */
	 ...
	 * 
	 * @param workerObserverClass
	 *            WorkerObserver class to add.
	 */
	 ...
	 * 
	 * @param klass
	 *            GiraphJobObserver class to set.
	 */
	 ...
	 * 
	 * @param name
	 *            String key name to use.
	 * @param xface
	 *            interface of the classes being set.
	 * @param klasses
	 *            Classes to set.
	 */
	 ...
	 * 
	 * @param doOutputDuringComputation
	 *            True iff we want output to happen during computation
	 */
	 ...
	 * 
	 * @param vertexOutputFormatThreadSafe
	 *            True iff selected VertexOutputFormat is thread-safe
	 */
	public final void setVertexOutputFormatThreadSafe(
			boolean vertexOutputFormatThreadSafe) {
		VERTEX_OUTPUT_FORMAT_THREAD_SAFE
				.set(this, vertexOutputFormatThreadSafe);
	}
	 ...
	 * application. Will be used only if {#vertexOutputFormatThreadSafe} is
	 * true.
	 * 
	 * @param numOutputThreads
	 ...
	 * 
	 * @param graphPartitionerFactoryClass
	 *            Determines how the graph is partitioned
	 */
	 ...
	 * 
	 * @param vertexResolverClass
	 *            Determines how vertex mutations are resolved
	 */
	 ...
	 * 
	 * @param v
	 *            true if we should create vertices when they get messages.
	 */
	 ...
	 * 
	 * @param partitionContextClass
	 *            Determines what code is executed for each partition before and
	 ...
	 * 
	 * @param workerContextClass
	 *            Determines what code is executed on a each worker before and
	 *            after each superstep and computation
	 */
	 ...
	 * 
	 * @param aggregatorWriterClass
	 *            Determines how the aggregators are written to file at the end
	 *            of the job
	 */
	 ...
	 *            Determines the why partitions are stored
	 */
	...
	/**
	 * Set worker configuration for determining what is required for a
	 * superstep.
	 * 
	 * @param minWorkers
	 *            Minimum workers to do a superstep
	 * @param maxWorkers
	 *            Maximum workers to do a superstep (max map tasks in job)
	 * @param minPercentResponded
	 *            0 - 100 % of the workers required to have responded before
	 *            continuing the superstep
	 */
	public final void setWorkerConfiguration(int minWorkers, int maxWorkers,
	...
	/**
	 * Utilize an existing ZooKeeper service. If this is not set, ZooKeeper will
	 * be dynamically started by Giraph for this job.
	 * 
	 * @param serverList
	 *            Comma separated list of servers and ports (i.e.
	 *            zk1:2221,zk2:2221)
	 */
	...
	/**
	 * Jars required in "Pure YARN" jobs (names only, no paths) should be listed
	 * here in full, including Giraph framework jar(s).
	 * 
	...
	/**
	 * Get heap size (in MB) for each task in our Giraph job run, assuming this
	 * job will run on the "pure YARN" profile.
	 * 
	...
	/**
	 * Set heap size for Giraph tasks in our job run, assuming the job will run
	 * on the "pure YARN" profile.
	 * 
	 * @param heapMb
	 *            the heap size for all tasks
	 */
	...
	/**
	 * The number of server tasks in our ZK quorum for this job run.
	 * 
	 ...
	 * 
	 * @param classPath
	 *            Classpath for the ZooKeeper jar
	 */
	...
	public int getMapTasks() {
		...
		if (mapTasks == -1) {
			throw new IllegalStateException(
					"getMapTasks: Failed to get the map " + "tasks!");
		}
		return mapTasks;
	}
	 ...
	 * 
	 * @param numInputSplitsThreads
	 *            Number of input split threads to use
	 */
	 ...
	 * 
	 * @param useUnsafeSerialization
	 *            If true, use unsafe serialization
	 */
	 ...
	 *            How often to checkpoint (0 means never)
	 */
	...
	/**
	 * Get the checkpoint frequeuncy of how many supersteps to wait before
	 * checkpointing
	 * 
	 ...
	 * 
	 * @param maxTaskAttempts
	 *            Max task attempts to use
	 */
	 ...
	 * 
	 * @param eventWaitMsecs
	 *            Number of milliseconds to wait for an event before continuing
	 *            on
	 */
	 ...
	 * minimum number of workers before a superstep.
	 * 
	 * @param maxMasterSuperstepWaitMsecs
	 *            Maximum milliseconds to wait before giving up trying to get
	 *            the minimum number of workers before a superstep
	 */
	...
	public void configureHadoopSecurity() {
		String hadoopTokenFilePath = System
	...
	public String getLocalHostname() throws UnknownHostException {
		return DNS.getDefaultHost(GiraphConstants.DNS_INTERFACE.get(this),
				GiraphConstants.DNS_NAMESERVER.get(this));
	}

	/**
	 * Set the maximum number of supersteps of this application. After this many
	 * supersteps are executed, the application will shutdown.
	 * 
	 * @param maxNumberOfSupersteps
	...
	/**
	 * Get the maximum number of supersteps of this application. After this many
	 * supersteps are executed, the application will shutdown.
	 * 
	...
	/**
	 * in edgeCentricMode, reuse byteArrayVertexIdEdges or not.
	 * @return
	 */
	public boolean reuseByteArrayVIDEdges() {
		return REUSE_BYTE_ARRAY_VID_EDGES.get(this);
	}
