...
public class ImmutableClassesGiraphConfiguration<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
	...
	public ImmutableClassesGiraphConfiguration(Configuration conf) {
		...
		try {
			vertexValueFactory = (VertexValueFactory<V>) classes
					.getVertexValueFactoryClass().newInstance();
	 ...
	 */
	public static <I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable> ImmutableClassesGiraphConfiguration<I, V, E, M> create(
			Configuration conf) {
	...
	public GraphPartitionerFactory<I, V, E, M> createGraphPartitioner() {
		Class<? extends GraphPartitionerFactory<I, V, E, M>> klass = classes
	...
	public VertexInputFormat<I, V, E> createVertexInputFormat() {
		Class<? extends VertexInputFormat<I, V, E>> klass = classes
	...
	public VertexOutputFormat<I, V, E> createVertexOutputFormat() {
		Class<? extends VertexOutputFormat<I, V, E>> klass = classes
	...
	public VertexResolver<I, V, E, M> createVertexResolver(
		...
		VertexResolver<I, V, E, M> resolver = ReflectionUtils.newInstance(
				getVertexResolverClass(), this);
	...
	public WorkerContext createWorkerContext(GraphState<I, V, E, M> graphState) {
		WorkerContext workerContext = ReflectionUtils.newInstance(
				getWorkerContextClass(), this);
	...
	public ReusableEdge<I, E> createReusableEdge() {
		if (isEdgeValueNullWritable()) {
			return (ReusableEdge<I, E>) EdgeFactory
					.createReusable(createVertexId());
		} else {
			return EdgeFactory.createReusable(createVertexId(),
					createEdgeValue());
		}
	}
	...
	/**
	 * Get the user's subclassed {@link org.apache.giraph.edge.OutEdges} used
	 * for input
	 * 
	 ...
	 */
	public Partition<I, V, E, M> createPartition(int id,
			Progressable progressable) {
		Class<? extends Partition<I, V, E, M>> klass = classes
				.getPartitionClass();
		Partition<I, V, E, M> partition = ReflectionUtils.newInstance(klass,
				this);
	 ...
	 * 
	 * @param buf
	 *            Buffer to use for the output (reuse perhaps)
	 * @param pos
	 *            How much of the buffer is already used
	 * @return ExtendedDataOutput object
	 */
	 ...
	 */
	public ExtendedDataInput createExtendedDataInput(byte[] buf, int off,
			int length) {
		if (useUnsafeSerialization) {
